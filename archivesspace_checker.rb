...
class ArchivesspaceChecker < Sinatra::Base
  ...
  CONFIG = if File.exist?(File.join('config', 'config.yml'))
  ...
  parser_options = Saxon::Processor.default.to_java.getUnderlyingConfiguration.parseOptions.
                     with_parser_feature("http://apache.org/xml/features/disallow-doctype-decl", true).
                     with_parser_feature("http://xml.org/sax/features/external-general-entities", false).
                     with_parser_feature("http://xml.org/sax/features/external-parameter-entities", false)
  ...
  def csv_output(xml, orig_name, out)
    ...
    out << CSV.generate_line( %w|type location line-number message|)
